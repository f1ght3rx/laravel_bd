<x-app-layout>
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-5">
            <x-filter :sort="$sort" :status="$status" :statuses="$statuses" />


            <div>
                <a href="{{ url('reports/create') }}"
                   class="inline-flex items-center rounded-lg bg-blue-600 px-4 py-2 text-white font-medium hover:bg-blue-700 transition-colors">
                    Создать заявку
                </a>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($reports as $report)
                    <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative min-h-[280px]">
                        ...
                        <div class="text-base font-medium text-slate-900 mt-auto pt-4 border-t border-slate-100">
                            Статус заявления:
